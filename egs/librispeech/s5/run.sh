...
if [ $stage -eq 6 ]; then
  for part in dev_clean test_clean; do
...
if [ $stage -eq 7 ]; then
  ...
  utils/subset_data_dir.sh --shortest data/dev_clean 2000 data/train_2kshort
  utils/subset_data_dir.sh data/dev_clean 2000 data/train_5k
  utils/subset_data_dir.sh data/dev_clean 2000 data/train_10k
fi
...
if [ $stage -eq 12 ]; then
  ...
  steps/align_fmllr.sh --nj 11 --cmd "$train_cmd" \
    data/dev_clean data/lang_nosp \
...
if [ $stage -eq 13 ]; then
  ...
  steps/get_prons.sh --cmd "$train_cmd" \
                     data/dev_clean data/lang_nosp exp/tri4b
...
if [ $stage -eq 14 ] && false; then
  ...
  steps/align_fmllr.sh --nj 5 --cmd "$train_cmd" \
    data/dev_clean data/lang exp/tri4b exp/tri4b_ali_clean_100
...
#   steps/make_mfcc.sh --cmd "$train_cmd" --nj 5 data/train_clean_360 \
...
if [ $stage -eq 16 ]; then
  ...
  steps/align_fmllr.sh --nj 20 --cmd "$train_cmd" \
                       data/dev_clean data/lang exp/tri4b exp/tri4b_ali_clean_460

  # create a larger SAT model, trained on the 460 hours of data.
  steps/train_sat.sh  --cmd "$train_cmd" 5000 100000 \
                      data/dev_clean data/lang exp/tri4b_ali_clean_460 exp/tri5b
fi
...
#   steps/make_mfcc.sh --cmd "$train_cmd" --nj 5 data/train_other_500 \
...
if [ $stage -eq 18 ]; then
  steps/align_fmllr.sh --nj 20 --cmd "$train_cmd" \
                       data/dev_clean data/lang exp/tri5b exp/tri5b_ali_960

  # train a SAT model on the 960 hour mixed data.  Use the train_quick.sh script
  # as it is faster.
  steps/train_quick.sh --cmd "$train_cmd" \
                       7000 150000 data/dev_clean data/lang exp/tri5b_ali_960 exp/tri6b
