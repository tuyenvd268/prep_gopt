...
def generate_sequence_training_data(in_feat_path, in_keys_path, in_label_path, out_feat_path, out_label_path, max_length=50):
    feat = load_feat(in_feat_path)
    keys = load_keys(in_keys_path)
    label = load_label(in_label_path)

    if not os.path.exists("resources/phoneme_dict.json"):
        phn_dict = gen_phn_dict(feat)
        with open("resources/phoneme_dict.json", "w", encoding="utf-8") as f:
            json_obj = json.dumps(phn_dict, indent=4, ensure_ascii=False)
            f.write(json_obj)
    else:
        phn_dict = json.loads(open("resources/phoneme_dict.json", "r"))

    feat, label = process_feat_seq(feat, keys, label, phn_dict, max_length=max_length)
